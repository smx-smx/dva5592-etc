...
printf "$table_format" "$(dup_char $row1)" "$(dup_char $row2)" "$(dup_char $row3)" "$(dup_char $row4)" "$(dup_char $row5)" "$(dup_char $row6)" "$(dup_char $row7)"
for obj in `cmclient GETO Device.NAT.PortMapping`
do
...
row=1
IFS=","
for proto in `cmclient GETV $obj.${prefix}AdditionalExternalPort`
do
if [ $row -eq 1 ]; then
...
set -f; IFS=","; set -- $ports; unset IFS; set +f
IFS=":" read -r proto port_range << EOM
$1
EOM
IFS="-" read -r a b << EOM
...
set -f; IFS=","; set -- $s; unset IFS; set +f
for p; do
if [ "$show" = "true" ]; then
out="$out,$p"
else

...
set -f; IFS=","; set -- $cur_list; unset IFS; set +f
for token; do
...
set -f; IFS="#"; set -- $args; unset IFS; set +f
for token; do
alias=${token#*"^"}
obj=${token%%"^"*}
...
set -f; IFS="#"; set -- $cust; unset IFS; set +f
for token; do
echo "\"$token\""
done
fi
...
for user in $normalUsers $powerUsers
do
format_print_obj_with_prop_ex $user
done
elif [ "$loggedType" = "AdminUser" ]; then
...
PowerUser)  cmclient -v normalUsers GETV "Device.Users.User.[X_ADB_Role=NormalUser].Username" # continuation below
cmclient -v powerUsers GETV "Device.Users.User.[X_ADB_Role=PowerUser].Username"   ;;
*)          exit 1 ;;
esac
for username in $normalUsers $powerUsers $allUsers
do

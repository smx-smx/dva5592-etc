...
upper_interfaces_get() {
	...
	while [ "$ll_obj_list" ]; do
		...
		ll_obj=""
		for obj in $(cmclient GETO -u "$_user" "Device.**.[LowerLayers,$ll_temp]"); do
...
remove_entry_from_all_owners() {
	...
	while [ "$removal_candidate_list" ]; do
		...
		removal_candidate_list="${removal_candidate_list#*${_seprator_}}"
		direct_owners=""
		for obj in $(cmclient GETO "Device.**.[LowerLayers,$single_candidate]"); do
...
show_split_text() {
	...
	local that="$2"
	this="$(
		sed -e 's/\./ /g' -e 's/VoIP/VOIP/g' -e 's/QoS/QOS/g' -e 's/^X_ADB_//' -e 's/\([a-z]\)\([A-Z][a-z]\{2,\}\)/\1 \2/g' -e 's|\([a-z]\)\([A-Z]\)|\1 \2|g' -e 's|\([A-Z][A-Z]\)\([A-Z][a-z]\)|\1  \2|g' -e 's/[ ]*/ /g' -e 's/^ //' <<EOF
$this
EOF
	)"
	that="$(
		sed -e 's/X_ADB_//g' <<EOF
$that
EOF
	)"
...
show_object() {
	...
	escaped_display_params=$(
		sed "s/\\\,/$comma_replacement/g" <<-EOF
			$display_params
		EOF
	)
	if [ "$show_all_except" = "yes" ]; then
		nospace_display_params=$(
			sed 's/,[ ]*/,/g; s/[ ]*::/::/g; s/^[ ]*//g' <<-EOF
				$escaped_display_params
			EOF
		)

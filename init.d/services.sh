...
WiFiKeyPassphrase$tail	$ap.Security.KeyPassphrase
do
if [ -z "$pf" ]; then pf="$p"; continue; fi
...
versionChanged=1
if [ -n "$prevVer" ]; then
prevMajor=${prevVer%%.*}; __prevVer=${prevVer#*.}
prevMinor=${__prevVer%%.*}; __prevVer=${__prevVer#*.}
prevSub=${__prevVer%%.*}; __prevVer=${__prevVer#*.}
prevBuild=${__prevVer%%.*}
prevCmp=`expr $prevMajor \* 256 \* 256 \* 256 \* 256 + $prevMinor \* 256 \* 256 \* 256 + $prevSub \* 256 \* 256 + $prevBuild`
curMajor=${curVer%%.*}; __curVer=${curVer#*.}
curMinor=${__curVer%%.*}; __curVer=${__curVer#*.}
curSub=${__curVer%%.*}; __curVer=${__curVer#*.}
curBuild=${__curVer%%.*}
...
service_echo_start "Port Mapping"; /etc/ah/NATPortMapping.sh init; service_echo_end "Port Mapping"
}
init_dev_route() {
...
( touch /tmp/loading_modules; load_modules /etc/modules.d/*; rm -f /tmp/loading_modules; post_moduleload) &
ifconfig lo 127.0.0.1 up
echo "127.0.0.1 localhost. localhost" > /tmp/hosts
[ -h /etc/hosts ] || (rm /etc/hosts; ln -s /tmp/hosts /etc/hosts)
...
init_procfs
start_swap
service_echo_start "Yatta Transport Fast Forwarding"; /etc/ah/Yatta.sh init; service_echo_end "Yatta Transport Fast Forwarding"
mkdir -p /tmp/dns
if [ -f "/usr/sbin/httpd" ]; then
mkdir /tmp/httpd
...
service_echo_start "NAT"; start_nat; service_echo_end "NAT"
...
(cmclient SET -u boot "Device.DHCPv4.Server.[Enable=true].Enable" "true"; service_echo_end "DHCP server") &
...
service_echo_start "WiFi Radio (B)"; wifiradio_sched_start; service_echo_end "WiFi Radio"
else

...
compute_occurence() {
	...
	for secs in $(
		cmclient GETV X_ADB_ParentalControl.RestrictedHosts.TimeOfDayProfile.TimeOfDay.UsagePeriodBegin
		cmclient GETV X_ADB_ParentalControl.RestrictedHosts.TimeOfDayProfile.TimeOfDay.UsagePeriodEnd
	); do
		min=$((($secs / 60) % 60))
		tok=$min
		secs=$(($secs % 60))

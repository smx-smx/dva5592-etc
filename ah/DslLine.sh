...
dsl_profile_2_hex() {
	...
	for arg; do
		case $arg in
		"8a")
			_profile=$(($_profile | 0x01))
			;;
		"8b")
			_profile=$(($_profile | 0x02))
			;;
		"8c")
			_profile=$(($_profile | 0x04))
			;;
		"8d")
			_profile=$(($_profile | 0x08))
			;;
		"12a")
			_profile=$(($_profile | 0x10))
			;;
		"12b")
			_profile=$(($_profile | 0x20))
			;;
		"17a")
			_profile=$(($_profile | 0x40))
			;;
		"30a")
			_profile=$(($_profile | 0x80))
			;;
		"35b")
			_profile=$(($_profile | 0x100))
			;;
		*) ;;
...
service_config() {
	...
	if [ "$newEnable" = "true" -a "$changedEnable" = 1 ]; then
		...
		if [ "$annex_autosense" = "true" ]; then
			...
			if [ "$autosense_fallback" = "Switch" ]; then
				...
				*)
				...
				if [ $autosense_timeout -le 43200 ]; then

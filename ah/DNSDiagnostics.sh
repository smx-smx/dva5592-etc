#!/bin/sh
AH_NAME="DNSDiag"
[ "$user" = "DNSDiagnostics_${obj}" ] && exit 0
...
[ -e /tmp/DNSDiagnostics_${obj}.pid ] && for pid in $(cat /tmp/DNSDiagnostics_${obj}.pid); do
	pkill -9 -P $pid
	kill $pid
done
...
if [ -n "$newInterface" ]; then
	...
	if [ -z "$interfaceName" ]; then
		...
		if [ -z "$newDNSServer" ]; then
			cmclient -v dnso GETO Device.DNS.Client.Server.*.[Enable=true]
			[ ${#dnso} -gt 0 ] && cmclient -v server GETV $dnso.DNSServer
...
perform_test() {
	...
	while [ "$lookupCount" -lt $newNumberOfRepetitions ]; do
		...
		cmclient -v resId ADD "$obj.Result"

...
wait_set_sched_affinity() {
	...
	*)
	...
	while [ ${#_pid} -eq 0 -a $((i = i - 1)) -gt 0 ]; do
		sleep 0.5
		_pid=$(pidof $1)
	done
	[ ${#_pid} -eq 0 ] && return 1
...
wifiradio_get_counters() {
	...
	while read -r line; do
		case "$line" in
		*"txerror"*)
			txerror=${line#*txerror }
			txerror=${txerror%% *}
			;;
		*"txnobuf"*)
			txnobuf=${line#*txnobuf }
			txnobuf=${txnobuf%% *}
			;;
		*"rxnobuf"*)
			rxnobuf=${line#*rxnobuf }
			rxnobuf=${rxnobuf%% *}
			;;
		*"rxfrmtoolong"*)
			rxfrmtoolong=${line#*rxfrmtoolong }
			rxfrmtoolong=${rxfrmtoolong%% *}
			rxfrmtooshrt=${line#*rxfrmtooshrt }
			rxfrmtooshrt=${rxfrmtooshrt%% *}
			rxinvmachdr=${line#*rxinvmachdr }
			rxinvmachdr=${rxinvmachdr%% *}
			rxbadfcs=${line#*rxbadfcs }
			rxbadfcs=${rxbadfcs%% *}
			break
			;;
		esac

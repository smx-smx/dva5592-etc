...
help98_add_dhcp_options() {
	local tr181_dhcp="$1"
	local tr098_dhcp="$2"
	for option in $(cmclient GETO "$tr181_dhcp.Option"); do
	...
	for static_addr in $(cmclient GETO "$tr181_dhcp.StaticAddress"); do
...
help98_build_lan_ipinterface() {
	...
	for ipv4 in $(cmclient GETO "$ipif.IPv4Address"); do
	...
	first_pool=1
	for dhcp in $(cmclient GETO "Device.DHCPv4.Server.Pool.*.[Interface=$ipif]"); do
...
help98_add_dhcpclient() {
	...
	if [ -n "$dhcp" ]; then
		...
		for req_opt in $(cmclient GETO "$dhcp.ReqOption"); do
		...
		for sent_opt in $(cmclient GETO "$dhcp.SentOption"); do
		...
		if [ "$o" = "atm" ]; then
			for atm in $(cmclient GETO "Device.ATM.Link.*.[LowerLayers=$chan]"); do
				...
				if [ "$link_type" = "EoA" ]; then
					...
					for eth_link in $(cmclient GETO "Device.Ethernet.Link.*.[LowerLayers=$atm]"); do
...
Do_WANDeviceForAustria() {
	...
	for wan_eth in $(cmclient GETO "Device.Ethernet.Interface.*.[Upstream=true]"); do
		...
		for eth_link in $(cmclient GETO "Device.Ethernet.Link.*.[LowerLayers=$wan_eth]"); do
...
for bridge in $(cmclient GETO "Device.Bridging.Bridge"); do
	...
	for port in $(cmclient GETO "$bridge.Port.*.[ManagementPort=false]"); do
...
for lan_eth in $(cmclient GETO "Device.Ethernet.Interface.*.[Upstream=false]"); do
...
for wlan in $(cmclient GETO "Device.WiFi.SSID"); do
...
for wan_usb in $(cmclient GETO "Device.USB.Interface.*.[Upstream=true]"); do
	wanDevId=$(help181_add_tr98obj "$OBJ_IGD.WANDevice")
	...
	for modemObj in $(cmclient GETO "Device.X_ADB_MobileModem.Interface.[LowerLayers=$wan_usb]"); do
...
for route in $(cmclient GETO "Device.Routing.Router.*.IPv4Forwarding"); do
...
for host in $(cmclient GETO "Device.Hosts.Host"); do
...
for qos_obj in App Classification Flow Policer Queue QueueStats; do
	for qos in $(cmclient GETO "Device.QoS.$qos_obj"); do

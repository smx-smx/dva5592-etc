...
generate_filename() {
	...
	*"%MACADDRESS%"*)
		...
		IFS=:
		for n in $mac_address_dotted; do
...
perform_download() {
	...
	if [ -n "$newFileName" ]; then
		if ! generate_filename "${newFileName}"; then
...
service_config() {
	if [ $changedEnable -eq 1 ]; then
		enable_dms "$newEnable" "$newCheckInterval"
	elif [ $changedCheckInterval -eq 1 ]; then
		update_check_interval "$newEnable" "$newCheckInterval"
	fi
	if [ $changedPerformDownload -eq 1 -a "$newPerformDownload" = "true" ]; then

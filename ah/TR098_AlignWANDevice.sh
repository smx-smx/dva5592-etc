...
local _mode="$2"
remove_reference
for cmd in `cmclient GETA InternetGatewayDevice.WANDevice. | grep -v ";0" `
do 
...
do
for wan_dev in `cmclient GETO "InternetGatewayDevice.WANDevice.*.WANDSLInterfaceConfig.[X_ADB_TR181Name=$dsl]"`
do
...
;;
"eth")
for eth_intf in `cmclient GETO "Device.Ethernet.Interface.*.[Upstream=true]"`
do
for wan_dev in `cmclient GETO "InternetGatewayDevice.WANDevice.*.WANEthernetInterfaceConfig.[X_ADB_TR181Name=$eth_intf]"`
do
wan_id="${wan_dev##*WANDevice.}"
wan_id="${wan_id%%.*}"
help98_destroy_wanconnection "$wan_id"
done
for ip_int in `cmclient GETO Device.IP.Interface.[Alias="$profile"]`
do

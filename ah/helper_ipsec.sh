...
'
	...
	for tparam in $parameter_list; do
		tvar=${tparam%;*}
		tval=${tparam#*;}
		case "$tvar" in
...
_exec_fullreconf() {
	...
	_exec_ikecleanup
	for filter_obj in $parameter_list; do
...
help_genpsk() {
	...
	for sec_obj_index in $(cmclient GETV Device.IPsec.X_ADB_Security.*.[Enable="true"].[Side="Remote"].Order | sort); do
...
help_gensetkey() {
	...
	echo "flush;"
	echo "spdflush;"
	for sp_file in $SETKEY_INCLUDE_PATH/*; do
		echo ""
		echo "# $sp_file"
		cat "$sp_file"
	done
...
help_genracoon() {
	...
	echo "}"
	for inc_info in $RACOON_INCLUDE_PATH/*; do

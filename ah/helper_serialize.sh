...
help_serialize() {
	...
	mkdir "${lockdir}-$$.pid" 2>/dev/null || return
	...
	echo "${lockdir}"
	if [ $trap -eq 1 ]; then
		help_append_trap "rmdir \"${lockdir}\" \"${lockdir}-$$.pid\" 2>/dev/null" "EXIT"
		trap "exit" INT TERM
	fi
}
help_serialize_nowait() {
	...
	local x
	if [ "$2" != "notrap" ]; then
		help_append_trap "rmdir \"${lockdir}\" 2>/dev/null" "EXIT"
		trap "exit" INT TERM
	fi
	until mkdir "${lockdir}" 2>/dev/null; do
		i=$((i + 1))
...
help_serialize_unlock() {
	...
	local lockdir="/tmp/lock/${1:-ah_${AH_NAME}${obj}}"
	rmdir "${lockdir}" "${lockdir}-$$.pid" 2>/dev/null
	:
}

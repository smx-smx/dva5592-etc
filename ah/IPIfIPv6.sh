...
service_reconf_ipv6_prefix() {
	for ipv6_obj in $(cmclient GETO "$ip_interface.IPv6Address.[Prefix=$obj].[Enable=true]"); do
...
service_reconf_ipv6_static_pd() {
	...
	if [ "$newEnable" = "true" ]; then
		for ipv6_obj in $(cmclient GETO "Device.IP.Interface.[X_ADB_Upstream=false].IPv6Prefix.[ParentPrefix=$obj].[Enable=true]"); do
...
service_delete_ipv6() {
	...
	case "$obj" in
	*"IPv6Prefix"*)
		for ipv6_addr in $(cmclient GETO "$ip_interface.IPv6Address.[Prefix=$obj]"); do

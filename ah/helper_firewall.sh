...
isEnslaved() {
	local ifname="$1" i l
	cmclient -v i GETO "Device.**.[Name=$ifname]"
	for i in $i; do
		if [ "${i%%.[0-9]*}" != "Device.Bridging.Bridge" ]; then
			cmclient -v l GETV InterfaceStack.[LowerLayer="$i"].HigherLayer
			for l in $l; do
...
help_bridge_get_wan() {
	...
	[ "$1" != "_p" ] && local _p
	_v=""
	cmclient -v _p GETV -u "$user" "$2.**.[ManagementPort=false].LowerLayers"
	for _p in $_p; do
...
getMapOfRule() {
	mapList=""
	local chain_nr=''
	cmclient -v chains GETO "Device.Firewall.Chain.Rule."
	local target targetChain
	for chains in $chains; do
		cmclient -v target GETV $chains.Target
		if [ $target = "TargetChain" ]; then
			cmclient -v targetChain GETV $chains.TargetChain
			cmclient -v enable GETV $chains.Enable
			if [ $enable = "true" ]; then
...
searchLoop() {
	...
	if [ 1 -eq $startPointIsOnMap ]; then
		...
		while [ $chainOccurence -gt 0 -a $result != "LOOP" ]; do

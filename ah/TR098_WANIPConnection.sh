...
service_set_param() {
	...
	"AddressingType")
		...
		if [ "$val98" = "DHCP" ]; then
			...
			for ipv4_obj in $(cmclient GETO "$tr181obj.IPv4Address.[AddressingType=Static]"); do
			...
			for ipv4_obj in $(cmclient GETO "$tr181obj.IPv4Address.[AddressingType=DHCP]"); do
				help181_del_object "$ipv4_obj"
			done
		fi
		;;
	"DefaultGateway")
		...
		;;
	"DNSServers")
		for trgt in $(cmclient GETV Device.DNS.Client.Server.*.[Interface=$tr181obj].[Type=Static].DNSServer); do
			case $val98 in
			*"$trgt"*) ;;
...
service_get() {
	...
	"ConnectionStatus")
		...
		;;
	"DNSServers")
		for trgt in $(cmclient GETO Device.DNS.Client.Server.*.[Interface=$tr181obj]); do
...
service_config() {
	for i in AddressingType DefaultGateway DNSServers; do
...
service_delete() {
	for layer3_obj in $(cmclient GETO "InternetGatewayDevice.Layer3Forwarding.Forwarding.[Interface=$obj]"); do
	...
	for dns_obj in $(cmclient GETO "Device.DNS.Client.Server.*.[Interface=$tr181obj]"); do
		help181_del_object "$dns_obj"
	done
	for relay_obj in $(cmclient GETO "Device.DNS.Relay.Forwarding.*.[Interface=$tr181obj]"); do

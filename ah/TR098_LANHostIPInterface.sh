...
service_link_stack() {
	...
	for if_key in $(cmclient GETV "InternetGatewayDevice.Layer2Bridging.AvailableInterface.*.[InterfaceReference>$tr98_landevice].AvailableInterfaceKey"); do
...
service_delete() {
	...
	if [ -n "$found_obj" ]; then
		...
		if [ -n "$lanhost_search" ]; then
			for is_ipv4 in $(cmclient GETO "$ipv4_search.IPv4Address.*.[$PARAM_TR098>$lanhost_search]"); do
				ipv4_found=1
				break
			done

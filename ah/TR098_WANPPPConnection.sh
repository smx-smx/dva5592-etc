...
service_set_param() {
	...
	"DefaultGateway")
		...
		;;
	"DNSServers")
		for trgt in $(cmclient GETV Device.DNS.Client.Server.*.[Interface=$ip_obj].[Type=Static].DNSServer); do
			case $val98 in
			*"$trgt"*) ;;
...
service_get() {
	...
	case $param98 in
	"DNSServers")
		for trgt in $(cmclient GETO Device.DNS.Client.Server.*.[Interface=$ip_obj]); do
...
service_config() {
	for i in DefaultGateway DNSServers; do
...
service_delete() {
	...
	for dns_obj in $(cmclient GETO "Device.DNS.Client.Server.*.[Interface=$ip_obj]"); do
		help181_del_object "$dns_obj"
	done
	for relay_obj in $(cmclient GETO "Device.DNS.Relay.Forwarding.*.[Interface=$ip_obj]"); do

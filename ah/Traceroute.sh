...
*)
	if [ -n "$newInterface" ]; then
		if [ "$newProtocolVersion" = "IPv6" ]; then
			cmclient -v sourceAddrs GETV "$newInterface.IPv6Address.[Status=Enabled].[IPAddressStatus=Preferred].IPAddress"
			...
			cmclient -v sourceAddr GETV "$newInterface.IPv4Address.1.IPAddress"
		fi
...
(
	...
	error=1
	set -f
	traceroute $verOption $interfaceOption $countOption $timeoutOption $maxHopOption $dscpOption $destHost 2>&1 | (
		while read -r row; do
			case $row in
			*"!H"*)
				error=1
			...
			*"ms"*)
				...
				cmclient -v idx ADD "$obj.RouteHops"

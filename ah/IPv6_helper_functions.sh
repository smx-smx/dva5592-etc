...
for radvd_intf in `cmclient GETO "Device.RouterAdvertisement.[Enable=true].InterfaceSetting.*.[Enable=true].[Interface=$ipif].[ManualPrefixes>$prefix]"`
do
if [ "$op" = "DEL" ]; then
manprefixes=""
set -f
...
for radvd_intf in `cmclient GETO "Device.RouterAdvertisement.[Enable=true].InterfaceSetting.*.[Enable=true].[Interface=$ipif]"`
do
if [ "$op" = "DEL" ]; then

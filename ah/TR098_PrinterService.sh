...
"PrintQueueLength")
job_len=0
for i in `cmclient GETO "$found_obj.PrintJob.*.[Status=Queued]"`
do
let "job_len += 1"
done
for i in `cmclient GETO "$found_obj.PrintJob.*.[Status=Spooling]"`
do
let "job_len += 1"
done
for i in `cmclient GETO "$found_obj.PrintJob.*.[Status=Printing]"`
do
let "job_len += 1"
done
value98="$job_len"

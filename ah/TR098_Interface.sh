	...
	if [ -n "$value181" ]; then
		...
		"DownstreamInterfaces" | "UpstreamInterfaces")
			...
			for arg; do
				case "$arg" in
				*"Radio"*)
					for if098 in $(cmclient GETO "InternetGatewayDevice.LANDevice.WLANConfiguration.[X_ADB_TR181Name=$arg]"); do
						[ -n "$if098" ] && break
					done
					;;
				*)
...
service_config() {
	...
	for i in Interface Interfaces X_ADB_InboundInterface InboundInterface OutboundInterface UpstreamInterfaces DownstreamInterfaces \
		X_ADB_OutboundInterface X_DLINK_OutboundInterface Provider; do
